...
class CSVReader(object):   
    ...
    def GetSDDBs(self):   
        try:
            ...
            self.dtvt_log.info("Reading SDDBs from:  "+ self.csv_folder_path +"\\"+ self.c_sddbFile)
        except:
            self.dtvt_log.error("Module = CSVReader, Method=GetSDDBs"+ sys.exc_info()[0]) 
            return None

    def GetTracksData(self):
        try:
            self.dtvt_log.info("Reading Tracks from:  " + self.csv_folder_path +"\\"+ self.c_trackFile)
            return Tracks_Cap.Tracks_Cap( self.csv_folder_path  +"\\"+  self.c_trackFile)
        except:
             self.dtvt_log.error("Source Module: CSVReader, Method:GetTracksData"+ sys.exc_info()[0])
             return None 
    ...
    def GetRoutes(self):   
        try:
            ...
            self.dtvt_log.info("Reading Routess from:  "+ self.csv_folder_path +"\\"+ self.c_RoutesFile)
        except:
            self.dtvt_log.error("Module = CSVReader, Method = GetRoutes"+ sys.exc_info()[0]) 
            return None

    def GetPoints(self):
        try:
            return Points_Cap.Points_Cap(self.csv_folder_path +"\\"+ self.c_PointsFile)
            self.dtvt_log.info("Reading Points from:  "+ self.csv_folder_path +"\\"+ self.c_PointsFile)
        except:
            self.dtvt_log.error("Module = CSVReader, Method = GetPoints"+ sys.exc_info()[0]) 
            return None
    ...
    def GetSignalisationAreas(self):
        try:
            ...
            self.dtvt_log.info("Reading Signalisation_Areas from:  "+ self.csv_folder_path +"\\"+ self.c_SignalisationAreasFile)
        except:
            self.dtvt_log.error("Module = CSVReader, Method = GetSignalisationAreas "+ sys.exc_info()[0]) 
            return None
    ...
    def GetSecondaryDetectionDevices(self):
       try:
           ...
           self.dtvt_log.info("Reading Signalisation_Areas from:  "+ self.csv_folder_path +"\\"+ self.c_SecondaryDetectionDevicesFile)
       except:
           self.dtvt_log.error("Module = CSVReader, Method = GetSecondaryDetectionDevices "+ sys.exc_info()[0]) 
           return None

    def GetSwitchs(self,CapFilePath):
       try:
           return Switchs_Cap.Switchs_Cap(self.csv_folder_path +"\\"+ self.c_SwitchsFile)
           self.dtvt_log.info("Reading Switchs from:  "+ self.csv_folder_path +"\\"+ self.c_SwitchsFile)
       except:
           self.dtvt_log.error("Module = CSVReader, Method = GetSwitchs "+ sys.exc_info()[0]) 
           return None
